...
public class EditProgrammeSessionCommandValidatorTests
{
    private EditProgrammeSessionCommandValidator _validator = null!;

    [SetUp]
    public void SetUp()
    {
        _validator = new EditProgrammeSessionCommandValidator();
    }

    [Test]
    public void ShouldPassValidation_WhenIdsArePositive()
    {
        var result = _validator.Validate(new EditProgrammeSessionCommand { UserProgrammeId = 1, ProgrammeSessionId = 1 });

        result.IsValid.ShouldBeTrue();
    }

    private static EditProgrammeSessionCommand CommandWith(params LogWorkoutSetDto[] sets) => new()
    {
        UserProgrammeId = 1,
    ...
    {
        var result = _validator.Validate(new EditProgrammeSessionCommand { UserProgrammeId = id, ProgrammeSessionId = 1 });

        result.IsValid.ShouldBeFalse();
        result.Errors.ShouldContain(e => e.PropertyName == nameof(EditProgrammeSessionCommand.UserProgrammeId));
    }
    ...
    {
        var result = _validator.Validate(new EditProgrammeSessionCommand { UserProgrammeId = 1, ProgrammeSessionId = id });

        result.IsValid.ShouldBeFalse();
        result.Errors.ShouldContain(e => e.PropertyName == nameof(EditProgrammeSessionCommand.ProgrammeSessionId));
    }
}
